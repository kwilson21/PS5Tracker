...
@router.post("/login")
def login(data: OAuth2PasswordRequestForm = Depends()) -> Dict[str, str]:  # noqa: B008
    email = data.username
...
@router.get("/")
def read_users(skip: int = 0, limit: int = 100) -> List[schemas.User]:
    users = user_store.get_users(skip=skip, limit=limit)
    return users


@router.get("/{user_id}", response_model=schemas.User)
def read_user(user_id: int) -> models.User:
